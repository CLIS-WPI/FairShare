...
help:
	@echo "FairShare DSS LEO - Makefile Commands"
	@echo ""
...
docker-run:
	docker run --gpus all -it -v $(PWD):/workspace --name fairshare-dss-dev fairshare-dss:latest /bin/bash

docker-run-cpu:
	docker run -it -v $(PWD):/workspace --name fairshare-dss-dev fairshare-dss:latest /bin/bash

docker-stop:
	docker stop fairshare-dss-dev || true
	docker rm fairshare-dss-dev || true
